...
GraphInterface::GraphInterface(int x, int y, int w, int h)
{
   ...
   /* m_top_box.add_child(m_main_box);
    ...
    m_tool_box.set_dim(400,50);
    m_tool_box.set_gravity_xy(grman::GravityX::Left, grman::GravityY::Up);
    m_tool_box.set_bg_color(VERT_FOND);

    m_top_box.add_child(m_bt_box);
    m_bt_box.set_dim(200,15);
    m_bt_box.set_gravity_xy(grman::GravityX::Left, grman::GravityY::Down);
    m_bt_box.set_bg_color(VERT_FOND);
    ...
    m_bt_box.add_child(m_bt_supprimer_vertex);
    m_bt_supprimer_vertex.set_dim(75,10);
    m_bt_supprimer_vertex.set_gravity_xy(grman::GravityX::Right, grman::GravityY::Up);
    m_bt_supprimer_vertex.set_bg_color(VERT_FOND);

    m_bt_supprimer_vertex.add_child(m_text_bt_supprimer_vertex);
    m_text_bt_supprimer_vertex.set_dim(75,10);
    m_text_bt_supprimer_vertex.set_gravity_xy(grman::GravityX::Right, grman::GravityY::Up);
    m_text_bt_supprimer_vertex.set_bg_color(BLANC);
    m_text_bt_supprimer_vertex.set_message("SupVertx");



    ///Bouton supp arc

    m_bt_box.add_child(m_bt_supprimer_edge);
    m_bt_supprimer_edge.set_dim(75,10);
    m_bt_supprimer_edge.set_gravity_xy(grman::GravityX::Left, grman::GravityY::Down);
    m_bt_supprimer_edge.set_bg_color(VERT_FOND);

    m_bt_supprimer_edge.add_child(m_text_bt_supprimer_edge);
    m_text_bt_supprimer_edge.set_dim(75,10);
    m_text_bt_supprimer_edge.set_gravity_xy(grman::GravityX::Left, grman::GravityY::Down);
    m_text_bt_supprimer_edge.set_bg_color(BLANC);
    m_text_bt_supprimer_edge.set_message("SupSommet");
...
void Graph::bouton_supprimer_vertex()
{
    if(m_interface->m_bt_supprimer_vertex.clicked())
    {
        ...
        bool toto = true;
            ...
            {
                std::cout << "N° : "<< elem.first << std::endl;
            }
            while (toto==true)
            {
                ...
                if(it!=m_vertices.end())
                {
                    ...
                    std::cout << "Veuillez selectionner un n° valide" << std::endl;
                }
            }
...
void Graph::bouton_supprimer_edge()
{
    ...
    if(m_interface->m_bt_supprimer_edge.clicked())
    {
        ...
        {
            std::cout << "Num : "<< elem.first << "arete de " << elem.second.m_from <<" a "  <<  elem.second.m_to << std::endl;
        }
        while (toto==true)
        {
            ...
            {
                std::cout << "  Veuillez selectionner une arete valide" << std::endl;
            }
        }
        // appel de la fonction remove avec en paramètre l'indice de l'arc à supprimer
